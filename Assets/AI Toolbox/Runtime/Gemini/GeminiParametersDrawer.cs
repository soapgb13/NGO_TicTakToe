...
namespace AiToolbox {
[CustomPropertyDrawer(typeof(GeminiParameters))]
public class GeminiParametersDrawer : PropertyDrawer {
    private const string ApiKeyPrefix = "AI";
    private float _height;

    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
        label.text = "Gemini Parameters";
        label.tooltip = "Settings for the Gemini API.";
        ...
        // Question mark button
        {
            ...
            if (GUI.Button(buttonRect, icon, EditorStyles.label)) {
                Application.OpenURL("https://ai-toolbox.dustyroom.com/gemini-runtime/");
            }
        }
        ...
        if (property.isExpanded) {
            ...
            // API Key
            {
                var apiKeyProperty = property.FindPropertyRelative(nameof(GeminiParameters.apiKey));
                ...
                _height += rect.height + EditorGUIUtility.standardVerticalSpacing;
                var encryptionProperty = property.FindPropertyRelative(nameof(GeminiParameters.apiKeyEncryption));
                ...
                var apiKeyLabel = new GUIContent(apiKeyTitle, "Your API Key from Google AI Studio.");
                var isEncrypted = encryptionProperty.enumValueIndex == (int)ApiKeyEncryption.LocallyEncrypted;
                var passwordProperty = property.FindPropertyRelative(nameof(GeminiParameters.apiKeyEncryptionPassword));
                if (isEncrypted && !string.IsNullOrEmpty(apiKey) && !apiKey.StartsWith(ApiKeyPrefix) &&
            ...
            // API Key encryption
            {
                ...
                if (encryptionProperty.enumValueIndex == (int)ApiKeyEncryption.None &&
                    !string.IsNullOrEmpty(property.FindPropertyRelative(nameof(GeminiParameters.apiKey)).stringValue)) {
                ...
                if (EditorGUI.EndChangeCheck()) {
                    ...
                    var passwordProperty =
                        property.FindPropertyRelative(nameof(GeminiParameters.apiKeyEncryptionPassword));
                    var isEncrypted = encryptionProperty.enumValueIndex == (int)ApiKeyEncryption.LocallyEncrypted;
                    if (isEncrypted) {
                        ...
                        if (apiKeyProperty.stringValue.StartsWith(ApiKeyPrefix) &&
                ...
                if (showRemoteConfigField) {
                    var remoteConfigKeyProperty =
                        property.FindPropertyRelative(nameof(GeminiParameters.apiKeyRemoteConfigKey));
            ...
            // Model
            {
                var modelProperty = property.FindPropertyRelative(nameof(GeminiParameters.model));
                var rect = new Rect(position.x, position.y + _height, width, EditorGUIUtility.singleLineHeight);
                var modelLabel = new GUIContent("Model", "The Gemini model to use.");
            ...
            // Role
            {
                var roleProperty = property.FindPropertyRelative(nameof(GeminiParameters.role));
            ...
            // Timeout
            {
                var timeoutProperty = property.FindPropertyRelative(nameof(GeminiParameters.timeout));
            ...
            // Throttle
            {
                var throttleProperty = property.FindPropertyRelative(nameof(GeminiParameters.throttle));
